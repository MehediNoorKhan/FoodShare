...
const PrivateRoute = ({ children }) => {
    ...
    if (loading) {
        ...
        if (path.includes("addfood")) return <AddFoodSkeleton />;
        if (path.includes("manage-food")) return <ManageFoodSkeleton />; // fixed path
        if (path.includes("membership")) return <MembershipSkeleton />;
        if (path.includes("myfoodrequest")) return <MyFoodRequestSkeleton />;
        if (path.includes("fooddetails")) return <FoodDetailsSkeleton />;

        return null; // fallback for unknown paths
    }
